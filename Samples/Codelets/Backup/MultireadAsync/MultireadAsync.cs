        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, int[]> pair in readerPort)
                {
                    ...
                    Boolean checkPort = (Boolean)r[i].ParamGet("/reader/antenna/checkPort");
                    String swVersion = (String)r[i].ParamGet("/reader/version/software");
                    if ((model.Equals("M6e Micro") || model.Equals("M6e Nano") || 
                        (model.Equals("Sargas") && (swVersion.StartsWith("5.1")))) 
                        && (false == checkPort) && readerPort[pair.Key] == null)
                    {
